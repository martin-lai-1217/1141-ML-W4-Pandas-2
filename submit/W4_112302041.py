...
def filter_and_analyze_data(df):
    ...
    top_series = df.loc[df["總分"].idxmax(), ["姓名", "總分"]]
    print(top_series)
...
if __name__ == "__main__":
    ...
    df = load_and_explore_data(INPUT_CSV)   # 任務一
    df = feature_engineering(df)            # 任務二
    result = filter_and_analyze_data(df)    # 任務三、四
    save_results(result["processed_df"], OUTPUT_CSV)  # 任務五
